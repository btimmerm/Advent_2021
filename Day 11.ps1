...
Function Add-Adjacent {
    ...
    Process {
        If ($Map["$($Y)_$($X)"].Energy -gt 9 -and -not $Map["$($Y)_$($X)"].Flashed) {
            $Map["$($Y)_$($X)"].Flashed = $true
            Add-Adjacent2 -X $X -Y $Y
        }
    }
}
...
ForEach ($Steps in 1..100) {
    ...
    # Count
    $Results += @($Map.Values | Where-Object {$_.Flashed}).count
