...
$OutputString = $Data.Split("`r`n") | ForEach-Object {($_.Split("|"))[1] }
$SignalLength = $OutputString.Split(" ") | ForEach-Object {$_.Trim().Length}

$Answer_Puzzle_1 = ($SignalLength | Group-Object -NoElement | Where-Object {$_.Name -In @(2, 3, 4, 7)} | Measure-Object Count -Sum).Sum
$Answer_Puzzle_1
...
$SegmentsByCount = @{}
$Numbers.Keys.ToCharArray() | Group-Object -NoElement | Group-Object Count | ForEach-Object {
    $SegmentsByCount.Add($_.Name, (($_.Group.Name | Sort-Object) -join ""))
}

$Codes = ForEach ($Puzzle in ($Data.Split("`r`n") | Where-Object {$_})) {
    $Parts = $Puzzle.Split("|")

    # Convert signal into object
    $SignalPatterns = ForEach ($Signal in ($Parts[0].Split(" ").Trim() | Where-Object {$_})) {
        [PSCustomObject]@{
            Signal  = ($Signal.ToCharArray() | Sort-Object) -join ""
            Decoded = Switch ($Signal.Length) {
                2 {"cf"} # 1
                3 {"acf"} # 7
                4 {"bcdf"} # 4
                7 {"abcdefg"} # 8
    ...
    # Create hash tables to store signal to LED and LED to Signal.
    $Signals = @{} # Signal to LED
    $Segments = @{} # LED to Signal
    ForEach ($Signal in ($SignalPatterns | Where-Object {$_.Decoded})) {
        $Signals.Add($Signal.Signal, $Signal.Decoded)
        $Segments.Add($Signal.Decoded, $Signal.Signal)
    }

    # Decode Signals by the number of times it was used in all 10 numbers.
    $SignalPatterns.Signal.ToCharArray() | Group-Object -NoElement | Group-Object Count | ForEach-Object {
        $Signal = ($_.Group.Name | Sort-Object) -join ""
        If (-not $Signals[$Signal]) {
            $Signals.Add($Signal, $SegmentsByCount[$_.Name])
            $Segments.Add($SegmentsByCount[$_.Name], $Signal)
        }
    }

    # What should turn on c
    $Segment_C = $Segments["cf"].ToCharArray() | Where-Object {$_ -ne $Segments["f"]}
    $Signals.Add("$($Segment_C)", "c")
    $Segments.Add("c", $Segment_C)

    # What should turn on a
    $Segment_A = $Segments["ac"].ToCharArray() | Where-Object {$_ -ne $Segments["c"]}
    $Signals.Add("$($Segment_A)", "a")
    $Segments.Add("a", $Segment_A)

    # What should turn on d
    $Segment_D = $Segments["bcdf"].ToCharArray() | Where-Object {$_ -notin @($Segments["b"], $Segments["c"], $Segments["f"]) }
    $Signals.Add("$($Segment_D)", "d")
    $Segments.Add("d", $Segment_D)

    # What should turn on g
    $Segment_G = $Segments["dg"].ToCharArray() | Where-Object {$_ -ne $Segments["d"]}
    $Signals.Add("$($Segment_G)", "g")
    $Segments.Add("g", $Segment_G)

    $OutputPatterns = $Parts[1].Split(" ").Trim() | Where-Object {$_}
    $Digits = ForEach ($Pattern in $OutputPatterns) {
        $Numbers[($Pattern.ToCharArray() | ForEach-Object {$Signals["$($_)"]} | Sort-Object) -Join ""]
    }

    [PSCustomObject]@{
        Pattern = $Parts[0]
        Output  = $Parts[1]
        Value   = [int]($Digits -join "")
    }
}

$Answer_Puzzle_2 = ($Codes.Value | Measure-Object -Sum).Sum
$Answer_Puzzle_2
