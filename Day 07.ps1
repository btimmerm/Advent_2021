...
[int[]]$Data = $ExampleInput.Split(",")
[int[]]$Data = $PuzzleInput.Split(",")

$Median = ($Data | Sort-Object)[$Data.Count / 2]
$Range = ($Median - 1)..($Median + 1)
$Results = ForEach ($Position in $Range) {
    $Fuel = @()
    ForEach ($Crab in $Data) {
        $Fuel += [Math]::Abs($Position - $Crab)
    }
    [PSCustomObject]@{
        Position = $Position
        Fuel     = ($Fuel | Measure-Object -Sum).Sum
    }
}

# Verify lowest fuel cost is in the middle of the range.
$Results | Sort-Object Fuel

$Answer_Puzzle_1 = $Results | Sort-Object Fuel | Select-Object -First 1 -ExpandProperty Fuel
$Answer_Puzzle_1
...
$Average = [math]::truncate(($Data | Measure-Object -Average).Average)
$Range = ($Average - 1)..($Average + 1)
$Results = ForEach ($Position in $Range) {
    $Fuel = @()
    ForEach ($Crab in $Data) {
        $Distance = [Math]::Abs($Position - $Crab)
        $Fuel += $Distance * ($Distance + 1) / 2 # Factorial
    }
    [PSCustomObject]@{
        Position = $Position
        Fuel     = ($Fuel | Measure-Object -Sum).Sum
...
$Answer_Puzzle_2 = $Results | Sort-Object Fuel | Select-Object -First 1 -ExpandProperty Fuel
$Answer_Puzzle_2
