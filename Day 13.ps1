...
# Example: First fold: "fold along y=7"
$Paper = @{}
$FoldLine = 7
ForEach ($Point in $Points) {
    $X, $Y = $Point
    If ([int]$Y -le $FoldLine) {
        Write-Verbose "$($X)_$($Y)" -Verbose
        $Paper["$($X)_$($Y)"] = $True
    } Else {
        Write-Verbose "$($X)_$($Y) becomes $($X)_$((2 * $FoldLine) - $Y)" -Verbose
        $Paper["$($X)_$((2 * $FoldLine) - $Y)"] = $True
    }
}
$Answer_Example = $Paper.Keys.Count
$Answer_Example
# Example: 17

# Puzzle 1: First fold: "fold along x=655"
$Paper = @{}
$FoldLine = 655
ForEach ($Point in $Points) {
    $X, $Y = $Point
    If ([int]$X -le $FoldLine) {
        ...
        #Write-Verbose "$($X)_$($Y) becomes $($X)_$((2 * $FoldLine) - $Y)" -Verbose
        $Paper["$((2 * $FoldLine) - $X)_$($Y)"] = $True
    }
}
$Answer_Puzzle_1 = $Paper.Keys.Count
$Answer_Puzzle_1
# Puzzle: 765
